...
{
	return rpm<DWORD>(baseAddress + signatures::dwEntityList + (index * 0x10));
}
...
{
	return rpm<DWORD>(baseAddress + signatures::dwGlowObjectManager);
}

bool Entity::isValid(DWORD playerBase)
{
	auto dormant = rpm<bool>(playerBase + signatures::m_bDormant);
...
DWORD Entity::getActiveWeapon(DWORD playerBase)
{
	...
	return rpm<DWORD>((baseAddress + signatures::dwEntityList + WeaponIndex * 0x10) - 0x10);
}
